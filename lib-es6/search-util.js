...
class SearchUtil {
  ...
  static parseCleverJSON(pkgs, result) {
    if (result.fileData) {
  ...
  static readCleverJSON(pkgs, basePath, source, file, json) {
    const defer = Q.defer();
  ...
  static readPackageJSON(basePath, source, file) {
    const defer = Q.defer();
