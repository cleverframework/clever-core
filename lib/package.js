...
function initPackages(CleverCore) {
  ...
  class Package {
    ...
    constructor(name) {
      ...
      this.viewsPath = `${this.config.clean.root}/packages/${this.name}/views`;
      this.assetsPath = `${this.config.clean.root}/packages/${this.name}/assets/dist`;
    ...
    requireRoute(args, route, app) {
      ...
      let mountPoint = cleverRoute.type === 'site' ? `/` : `/${cleverRoute.type}`;
