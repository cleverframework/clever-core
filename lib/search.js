  ...
  if (data) {
    try {
      ...
      if (json.clever) {
        ...
        let pathToCleverJSON = path.join(basePath, source, file, 'clever.json');
...
function fileForEachProcess (_packages, source, promises, file) {
  ...
  let filePath = path.join(basePath, source, file, 'package.json');
...
function searchSourceForFindPackages(_packages, disabled, source) {
  ...
  fs.readdir(path.join(basePath, source), processDirFilesFromSearchSource.bind(null, _packages, disabled, source, deferred));
