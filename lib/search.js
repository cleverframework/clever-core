...
function readModulePackageJSONFile(_modules, fileDefer, file, source, fileErr, data) {
  if (data) {
    try {
      let json = JSON.parse(data.toString());
      if (json.clever) {
        fs.readFile(path.join(process.cwd(), source, file, 'clever.json'), readModuleCleverJSONFileDone.bind(null, _modules, fileDefer, _modules.createModule(json.name,json.version,source), file, source));
        return;
      }
    } catch (err) {
